...
if (notInCluster.length) notYetReady.push("Nodes not included in cluster: " + notInCluster.join(", "));
...
if (unhealtyMembers.length) notYetReady.push("Has unhealty members: " + unhealtyMembers.join(", "));

// has primary node
if (!result.members.find(function (m) { return m.state === 1; })) notYetReady.push("Not found primary node");

result.ready = result.ok === 1 && notYetReady.length === 0;
if (notYetReady.length) result.notYetReady = notYetReady;
