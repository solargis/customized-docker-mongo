...
_check_version() {
	mongod --version | MIN_VERSION="$1" perl -ne '
	/^db version v(\d+)\.(\d+)\.(\d+)/ && (my ($a0, $a1, $a2) = ($1, $2, $3));
	END {
		my ($v0, $v1, $v2) = ($ENV{"MIN_VERSION"} =~ /^(\d+)(?:\.(\d+)(?:\.(\d+))?)?/);
		my $r = $a0 > $v0 || $a0 == $v0 && (!$v1 || $a1 > $v1 || $a1 == $v1 && (!$v2 || $a2 >= $v2)) ? 0 : 1;
		printf "%d.%d.%d (wanted %s) %s\n", $a0, $a1, $a2, $ENV{"MIN_VERSION"}, $r==0 ? "ok" : "fail";
		exit $r;
	}'
}

if [ "$(id -u)" -eq 0 ]; then
	for f in /docker-hooks.d/postboot-*.sh; do
...
if [ "$originalArgOne" = 'mongod' ]; then
	...
	if [ -n "$shouldPerformInitdb" ]; then
		...
		elif mongod --help 2>&1 | grep -q -- ' --sslMode '; then
		...
		if [ "$MONGO_INITDB_ROOT_USERNAME" ] && [ "$MONGO_INITDB_ROOT_PASSWORD" ]; then
			...
			echo "CREATE_ROOT_USER" "${mongo[@]}" "$rootAuthDatabase" "
				db.createUser({
	...
	if mongod --help 2>&1 | grep -q -- --bind_ip_all; then
		haveBindIp=
