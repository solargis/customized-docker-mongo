...
if (result.ok === 1 && result.members.length < result.config.members.length) {
	for (var i = 0; i < result.config.members.length; i++) {
		var member = result.config.members[i];
		if (!result.members.find(function (m) { return m.name === member.host; })) {
			var lastResult = applyWithLog("rs.add", function (m) { return rs.add(m); }, member);
			if (!lastResult.ok) break;
		}
	}
}
result.ready = result.ok === 1 && result.members.length === result.config.members.length && !!result.members.find(function (m) { return m.state === 1; });
